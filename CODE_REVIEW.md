...
| 优先级 | 编号 | 改进项 | 预估工作量 |
|--------|------|--------|-----------|
| 🔴 P0 | 1 | 添加认证机制 + CORS 白名单 + 升级 wss:// | 2-3 天 |
| 🔴 P0 | 2 | 删除 `subscribers/` 死代码目录 | 0.5 小时 |
| 🔴 P0 | 3 | 给共享状态加 `asyncio.Lock` | 1 天 |
| 🔴 P0 | 4 | 修复 ROS 时间戳覆盖（`node_manager.py:543`） | 0.5 小时 |
| 🔴 P0 | 5 | 前端：统一 WebSocket 实现（删除 useWebSocket） | 1-2 天 |
| 🔴 P0 | 6 | 前端：替换所有 `any` 为 discriminated union 类型 | 1 天 |
| 🔴 P0 | 7 | 添加核心路径的 pytest + Vitest 单元测试 | 3 天 |
| 🟡 P1 | 8-22 | 后端中等问题（共 15 项） | 2-3 天 |
| 🟡 P1 | 23-34 | 前端中等问题（共 12 项） | 2 天 |
| 🟢 P2 | 35-47 | 低优先级优化（共 13 项） | 3-5 天 |
...
1. **先处理 P0-1（安全）和 P0-5（前端 WS）** — 这两项改动面最大但影响最深远
2. **P0-2（删除死代码）可以作为热身前置任务** — 纯删除，零风险
3. **P0-4（时间戳）是单行修改** — 顺手修掉
4. **P0-6（any 类型）与 P0-5（WS 统一）有依赖** — 建议先统一 WS，再修类型
5. **P1 问题可批量处理** — 大部分是独立的局部修改
6. **每个 PR 加一条测试** — 逐步积累测试覆盖率

---

> 本报告由 CodeWhale 多维度并行审查生成，所有发现均有具体文件路径和行号支撑。
